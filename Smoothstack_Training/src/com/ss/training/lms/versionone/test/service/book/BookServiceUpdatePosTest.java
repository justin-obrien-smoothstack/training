package com.ss.training.lms.versionone.test.service.book;import static org.junit.Assert.assertEquals;
 ...
 *
 */
public class BookServiceUpdatePosTest {
	...
	public void test() { // passed
		stdIn.provideLines("1", "t", "Title1");
		BookService.getInstance(books, authors, publishers, scanner).update();
		assertEquals(books.get((long) 1).getTitle(), "Title1");
		assertEquals(books.get((long) 1).getAuthor(), 1);
		assertEquals(books.get((long) 1).getPublisher(), 1);
	}
}
